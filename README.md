...
## 功能
- 从多组 Google News RSS 查询抓取新线索，并按 `strict`、`fundraising`、`dex`、`ecosystem` 分组
- 低频接入 CoinGecko、DeFiLlama、CryptoRank、项目官网/Blog、RootData 缓存补数
- 自动抽取项目名称、赛道、地区、合规/机构/融资信号
- 对候选项目打分，生成可读理由
- 提供本地面板查看 Top leads、Watch Radar 和 Radar Pools
- 支持手动生成日报
...
- `data/projects.json`: 最近一次聚合后的候选项目，包含 `projects`、`strictProjects`、`looseProjects`、`fundraisingProjects`、`dexProjects`、`ecosystemProjects`
...
- `CryptoRank` 已接入自动缓存刷新，用于补充项目 rank、类别、市值/成交额，以及部分项目的融资轮次信息。
- 当前日报/Telegram 主输出仍以 `strictProjects` 为主，Radar Pools 更适合做观察池和人工跟进补充。
