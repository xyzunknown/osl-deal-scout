...
async function collectLeads(rootDir) {
  ...
  const sourceResults = await Promise.all((sources.rssQueries || []).map(async (source) => {
    try {
      ...
      return {
        ok: true,
        items: parseItems(xml).map((item) => ({ ...item, queryName: source.name, queryGroup: source.group || 'strict' }))
      };
    } catch (error) {
      return {
        ok: false,
  ...
  <div class="workbench">
    ...
    <div class="col-right" id="colRight">
      <div class="col-scroll">
        ...
        <div class="sidebar-section">
          <div class="sidebar-title">Radar Pools</div>
          <div id="radarPools"></div>
        </div>
        <div class="sidebar-section">
