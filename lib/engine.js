...
async function collectLeads(rootDir) {
  ...
  const radarBucketKeys = new Set([
    ...finalFundraisingProjects,
    ...finalDexProjects,
    ...finalEcosystemProjects
  ...
  const finalRadarProjects = sortProjectsForAction(
    dedupeProjectsByName(await enrichWithExternalSources(rootDir, config, enrichProjects(retainedRadarProjects, rootDir)))
  );
