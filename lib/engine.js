...
async function fetchExchangePairPrecision(pair) {
  ...
  if (!fetcher) {
    exchangePrecisionCache.set(cacheKey, null);
    return null;
  }
  try {
    ...
    const normalized = {
      pricePrecision: Number(result.pricePrecision),
      quantityPrecision: Number(result.quantityPrecision)
    };
    if (!Number.isFinite(normalized.pricePrecision) || normalized.pricePrecision < 0 || !Number.isFinite(normalized.quantityPrecision) || normalized.quantityPrecision < 0) {
      exchangePrecisionCache.set(cacheKey, null);
      return null;
    }
...
async function buildPairConfigRow(detail) {
  const candidates = pickUsdtPairCandidates(detail, await fetchCmcMarketPairs(detail.slug, { limit: 80 }));
  const defaultAmounts = {
    minOrderAmount: formatUsdAmount(PAIR_MIN_NOTIONAL_USD),
    maxOrderAmount: formatUsdAmount(PAIR_MAX_NOTIONAL_USD)
  };
  ...
  for (const pair of candidates) {
    ...
    const currentPrice = Number(pair.price || detail.priceUsd || 0);
    return {
      pair: `${pair.baseSymbol}/${pair.quoteSymbol}`,
      minTradeQuantity: currentPrice > 0 ? formatRoundedQuantity(PAIR_MIN_NOTIONAL_USD / currentPrice) : '—',
      maxTradeQuantity: currentPrice > 0 ? formatRoundedQuantity(PAIR_MAX_NOTIONAL_USD / currentPrice) : '—',
      ...defaultAmounts,
      pricePrecision: formatPrecisionStep(precision.pricePrecision),
      quantityPrecision: formatPrecisionStep(precision.quantityPrecision),
      currentPrice: formatPairPrice(currentPrice),
      precisionSourceExchange: pair.exchangeName || '',
      precisionSourceUrl: pair.marketUrl || '',
      precisionSourceResolved: true,
      remark: pair.exchangeName ? `该币种价格精度和数量精度取自${pair.exchangeName}交易所。` : ''
    };
  }

  const fallback = candidates[0];
  const fallbackPrice = Number(fallback.price || detail.priceUsd || 0);
  return {
    pair: `${fallback.baseSymbol}/${fallback.quoteSymbol}`,
    minTradeQuantity: fallbackPrice > 0 ? formatRoundedQuantity(PAIR_MIN_NOTIONAL_USD / fallbackPrice) : '—',
    maxTradeQuantity: fallbackPrice > 0 ? formatRoundedQuantity(PAIR_MAX_NOTIONAL_USD / fallbackPrice) : '—',
    ...defaultAmounts,
    pricePrecision: '—',
    quantityPrecision: '—',
    currentPrice: formatPairPrice(fallbackPrice),
    precisionSourceExchange: fallback.exchangeName || '',
    precisionSourceUrl: fallback.marketUrl || '',
    precisionSourceResolved: false,
    remark: fallback.exchangeName ? `暂未从${fallback.exchangeName}交易所接口拿到价格精度和数量精度。` : ''
  };
}
