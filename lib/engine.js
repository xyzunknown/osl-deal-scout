...
const cmcMarketPairsCache = new Map();
const indodaxPairsCache = { fetchedAt: 0, items: [] };
...
const PAIR_MIN_NOTIONAL_USD = 1;
const PAIR_MAX_NOTIONAL_USD = 17000;
const PAIR_MIN_NOTIONAL_IDR = 16000;
const PAIR_MAX_NOTIONAL_IDR = 170000000;
const exchangePrecisionCache = new Map();
...
function formatUsdAmount(value) {
  const num = Number(value || 0);
  if (!Number.isFinite(num) || num <= 0) return '—';
  return `${num.toLocaleString(undefined, { maximumFractionDigits: 0 })} USD`;
}
...
function formatRoundedQuantity(value) {
  const num = Number(value || 0);
  if (!Number.isFinite(num) || num < 0) return '—';
...
function formatTradeQuantity(value) {
  const num = Number(value || 0);
  if (!Number.isFinite(num) || num <= 0) return '—';
  if (num >= 1) return Math.round(num).toLocaleString(undefined, { maximumFractionDigits: 0 });
...
function countFractionDigits(value) {
  ...
  if (/e/i.test(raw)) {
    ...
    return countFractionDigits(num.toFixed(16).replace(/0+$/, '').replace(/\.$/, ''));
  }
  if (!raw.includes('.')) return 0;
  return raw.replace(/0+$/, '').split('.')[1]?.length || 0;
}

function formatPrecisionStep(value) {
...
function formatPriceByStep(value, step) {
  const num = Number(value || 0);
  if (!Number.isFinite(num) || num <= 0) return '—';
...
const CHAIN_CONFIG_PRESETS = [
  { match: /\btron\b/i, fullName: 'Tron(TRC20)', shortName: 'Tron' },
  { match: /\beth(ereum)?\b/i, fullName: 'Ethereum(ERC20)', shortName: 'Ethereum' },
  { match: /\bbnb|bsc|binance smart chain\b/i, fullName: 'BNB Smart Chain(BEP20)', shortName: 'BSC' },
  { match: /\bsolana\b/i, fullName: 'Solana', shortName: 'Solana' },
  { match: /\bbase\b/i, fullName: 'Base', shortName: 'Base' },
  { match: /\barbitrum\b/i, fullName: 'Arbitrum', shortName: 'Arbitrum' },
  { match: /\boptimism\b/i, fullName: 'Optimism', shortName: 'Optimism' },
  { match: /\bpolygon\b/i, fullName: 'Polygon', shortName: 'Polygon' },
  { match: /\bavalanche\b/i, fullName: 'Avalanche C-Chain', shortName: 'Avalanche' },
  { match: /\bton\b/i, fullName: 'TON', shortName: 'TON' },
  { match: /\baptos\b/i, fullName: 'Aptos', shortName: 'Aptos' },
  { match: /\bsui\b/i, fullName: 'Sui', shortName: 'Sui' },
  { match: /\bzksync\b/i, fullName: 'zkSync Era', shortName: 'zkSync' },
  { match: /\bkaia\b/i, fullName: 'Kaia', shortName: 'Kaia' },
  { match: /\bviction\b/i, fullName: 'Viction', shortName: 'Viction' }
];

const NATIVE_COIN_CHAIN_PRESETS = {
  bitcoin: {
    symbol: 'BTC',
    networkFullName: 'Bitcoin',
    networkShortName: 'Bitcoin',
    browserUrl: 'https://www.blockchain.com/explorer'
  },
  ethereum: {
    symbol: 'ETH',
  ...
  solana: {
    ...
    browserUrl: 'https://explorer.solana.com'
  },
  tron: {
    symbol: 'TRX',
  ...
  xrp: {
    ...
    networkShortName: 'XRP',
    browserUrl: 'https://livenet.xrpl.org'
  },
  bnb: {
    symbol: 'BNB',
    networkFullName: 'BNB Beacon Chain',
    networkShortName: 'BNB',
    browserUrl: 'https://explorer.bnbchain.org'
  },
  dogecoin: {
    symbol: 'DOGE',
    networkFullName: 'Dogecoin',
    networkShortName: 'DOGE',
    browserUrl: 'https://dogechain.info'
  },
  litecoin: {
    symbol: 'LTC',
    networkFullName: 'Litecoin',
    networkShortName: 'Litecoin',
    browserUrl: 'https://blockchair.com/litecoin'
  }
};

function resolveChainConfigPreset(platformName) {
  const rawName = String(platformName || '').trim();
  if (!rawName) {
    return { fullName: '', shortName: '', cmcUrl: '' };
  }
  const preset = CHAIN_CONFIG_PRESETS.find((item) => item.match.test(rawName));
  if (preset) {
    return {
...
function buildChainConfigRowCopyText(row) {
  return [
    row.tokenName,
    row.networkFullName,
...
function resolveNativeCoinChain(detail) {
  const slug = String(detail && detail.slug || '').trim().toLowerCase();
  const symbol = String(detail && detail.symbol || '').trim().toUpperCase();
  if (!slug && !symbol) return null;
  const preset = NATIVE_COIN_CHAIN_PRESETS[slug]
    || Object.values(NATIVE_COIN_CHAIN_PRESETS).find((item) => item.symbol === symbol)
    || null;
  if (!preset) return null;
  return {
    tokenName: symbol || preset.symbol || '',
    networkFullName: preset.networkFullName,
    networkShortName: preset.networkShortName,
    browserUrl: preset.browserUrl,
    contractAddress: '',
    cmcUrl: detail && detail.slug ? `https://coinmarketcap.com/currencies/${detail.slug}/` : '',
    platformName: preset.networkFullName,
    missingFields: [],
    rowCopyText: ''
  };
}

function buildChainConfig(detail) {
  const platforms = Array.isArray(detail && detail.platforms) ? detail.platforms : [];
  const coinCmcUrl = detail && detail.slug ? `https://coinmarketcap.com/currencies/${detail.slug}/` : '';
  if (!platforms.length) {
    const nativeRow = resolveNativeCoinChain(detail);
    if (nativeRow) {
      nativeRow.rowCopyText = buildChainConfigRowCopyText(nativeRow);
      return {
        rows: [nativeRow],
        remark: '该币种是原生币，使用主网浏览器地址，不使用合约地址。',
        missingRows: []
      };
    }
    return {
  ...
  const missingRows = [];
  const rows = platforms.reduce((list, platform) => {
    const tokenName = String(detail && detail.symbol || '').trim();
    const platformName = String(platform && platform.contractPlatform || '').trim();
    const browserUrl = String(platform && (platform.contractExplorerUrl || platform.contractBlockExplorerUrl) || '').trim();
    const contractAddress = String(platform && platform.contractAddress || '').trim();
    const preset = resolveChainConfigPreset(platformName);
    const missingFields = [];

    if (!platformName) missingFields.push('网络名称');
    if (!browserUrl) missingFields.push('币浏览器URL');
    if (!contractAddress) missingFields.push('合约地址');
    const row = {
      tokenName,
      networkFullName: preset.fullName,
      networkShortName: preset.shortName,
      browserUrl,
      contractAddress,
      cmcUrl: coinCmcUrl,
      platformName,
    ...
    row.rowCopyText = buildChainConfigRowCopyText(row);
    list.push(row);

    if (missingFields.length) {
      missingRows.push({
        name: preset.fullName || platformName || '未命名链',
        missingFields
      });
    }
    return list;
  ...
  let remark = '';
  if (rows.length > 1 && missingRows.length) {
    remark = `多链币种中，以下链路字段未完整抓取：${missingRows.map((item) => `${item.name}（缺少${item.missingFields.join('、')}）`).join('；')}。`;
  }
...
function normalizeExchangeSlug(value) {
  return String(value || '')
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
...
async function fetchCmcMarketPairs(slug, options = {}) {
  const normalizedSlug = String(slug || '').trim().toLowerCase();
  if (!normalizedSlug) return [];
  const limit = Math.max(20, Math.min(200, Number(options.limit || 80)));
  const cacheKey = `${normalizedSlug}:${limit}`;
  const cached = cmcMarketPairsCache.get(cacheKey);
  if (cached && (Date.now() - cached.fetchedAt) < CMC_DETAIL_TTL_MS) return cached.data;
  const url = `https://api.coinmarketcap.com/data-api/v3/cryptocurrency/market-pairs/latest?slug=${encodeURIComponent(normalizedSlug)}&start=1&limit=${limit}&category=spot&centerType=cex`;
  const payload = await fetchJson(url);
  const pairs = (((payload || {}).data || {}).marketPairs || []).map((pair) => ({
    rank: Number(pair.rank || 0),
    exchangeId: Number(pair.exchangeId || 0),
    exchangeName: String(pair.exchangeName || '').trim(),
    exchangeSlug: normalizeExchangeSlug(pair.exchangeSlug || pair.exchangeName || ''),
    marketPair: String(pair.marketPair || '').trim(),
    category: String(pair.category || '').trim().toLowerCase(),
    baseSymbol: String(pair.baseSymbol || '').trim().toUpperCase(),
    quoteSymbol: String(pair.quoteSymbol || '').trim().toUpperCase(),
    price: Number(pair.price || 0),
    volumeUsd: Number(pair.volumeUsd || 0),
    marketUrl: String(pair.marketUrl || '').trim()
  })).filter((pair) => pair.marketPair && pair.baseSymbol && pair.quoteSymbol);
  cmcMarketPairsCache.set(cacheKey, { fetchedAt: Date.now(), data: pairs });
  return pairs;
}

async function fetchIndodaxPairs() {
  if (indodaxPairsCache.items.length && (Date.now() - indodaxPairsCache.fetchedAt) < CMC_DETAIL_TTL_MS) {
    return indodaxPairsCache.items;
  }
  const payload = await fetchJson('https://indodax.com/api/pairs');
  const rows = (Array.isArray(payload) ? payload : []).map((item) => ({
    id: String(item.id || '').trim().toLowerCase(),
    symbol: String(item.symbol || '').trim().toUpperCase(),
    description: String(item.description || '').trim(),
    tradedCurrency: String(item.traded_currency_unit || item.traded_currency || '').trim().toUpperCase(),
    baseCurrency: String(item.base_currency || '').trim().toUpperCase(),
    pricePrecisionRaw: item.price_precision,
    priceScale: item.pricescale,
    tradeMinBaseCurrency: Number(item.trade_min_base_currency || 0),
    tradeMinTradedCurrency: Number(item.trade_min_traded_currency || 0),
    marketSuspended: Boolean(item.is_market_suspended),
    maintenance: Boolean(item.is_maintenance)
  })).filter((item) => item.id);
  indodaxPairsCache.fetchedAt = Date.now();
  indodaxPairsCache.items = rows;
  return rows;
}

async function fetchIndodaxTicker(pairId) {
  const normalized = String(pairId || '').trim().toLowerCase();
  if (!normalized) return null;
  const payload = await fetchJson(`https://indodax.com/api/ticker/${encodeURIComponent(normalized)}`);
  return payload && payload.ticker ? payload.ticker : null;
}

function pickIndodaxIdrPair(detail, indodaxPairs) {
  const symbol = String(detail && detail.symbol || '').trim().toUpperCase();
  return (indodaxPairs || []).find((pair) =>
    pair.baseCurrency === 'IDR' &&
    pair.tradedCurrency === symbol &&
    !pair.marketSuspended &&
    !pair.maintenance
  ) || null;
}

function pickUsdtPairCandidates(detail, marketPairs) {
  const symbol = String(detail && detail.symbol || '').trim().toUpperCase();
  const candidates = (marketPairs || []).filter((pair) =>
    pair.category === 'spot' &&
    pair.quoteSymbol === 'USDT' &&
    (!symbol || pair.baseSymbol === symbol)
  );
  const binance = candidates.filter((pair) => pair.exchangeSlug === 'binance');
  const others = candidates.filter((pair) => pair.exchangeSlug !== 'binance');
  return [...binance, ...others];
}

function parseBinancePrecision(symbolInfo) {
  const priceFilter = (symbolInfo.filters || []).find((item) => item.filterType === 'PRICE_FILTER');
  const lotSizeFilter = (symbolInfo.filters || []).find((item) => item.filterType === 'LOT_SIZE');
  if (!priceFilter || !lotSizeFilter) return null;
  return {
    pricePrecision: countFractionDigits(priceFilter.tickSize),
    quantityPrecision: countFractionDigits(lotSizeFilter.stepSize)
  };
}

async function fetchBinancePairPrecision(pair) {
  const symbol = `${pair.baseSymbol}${pair.quoteSymbol}`;
  const payload = await fetchJson(`https://api.binance.com/api/v3/exchangeInfo?symbol=${encodeURIComponent(symbol)}`);
  const info = Array.isArray(payload.symbols) ? payload.symbols[0] : null;
  return info ? parseBinancePrecision(info) : null;
}

async function fetchGatePairPrecision(pair) {
  const symbol = `${pair.baseSymbol}_${pair.quoteSymbol}`;
  const payload = await fetchJson(`https://api.gateio.ws/api/v4/spot/currency_pairs/${encodeURIComponent(symbol)}`);
  return {
    pricePrecision: Number(payload.precision || 0),
    quantityPrecision: Number(payload.amount_precision || 0)
  };
}

async function fetchIndodaxPairPrecision(pair) {
  const indodaxPairs = await fetchIndodaxPairs();
  const meta = indodaxPairs.find((item) => item.id === String(pair.exchangePairId || '').toLowerCase());
  if (!meta) return null;
  return {
    priceStep: String(meta.priceScale || meta.pricePrecisionRaw || '1'),
    quantityStep: meta.tradeMinTradedCurrency > 0 ? formatPrecisionStep(countFractionDigits(meta.tradeMinTradedCurrency)) : '1'
  };
}

async function fetchMexcPairPrecision(pair) {
  const symbol = `${pair.baseSymbol}${pair.quoteSymbol}`;
  const payload = await fetchJson(`https://api.mexc.com/api/v3/exchangeInfo?symbol=${encodeURIComponent(symbol)}`);
  const info = Array.isArray(payload.symbols) ? payload.symbols[0] : null;
  if (!info) return null;
  return {
    pricePrecision: Number(info.quotePrecision || info.quoteAssetPrecision || 0),
    quantityPrecision: Number(info.baseSizePrecision || info.baseAssetPrecision || 0)
  };
}

async function fetchBybitPairPrecision(pair) {
  const symbol = `${pair.baseSymbol}${pair.quoteSymbol}`;
  const payload = await fetchJson(`https://api.bybit.com/v5/market/instruments-info?category=spot&symbol=${encodeURIComponent(symbol)}`);
  const info = Array.isArray(payload?.result?.list) ? payload.result.list[0] : null;
  if (!info) return null;
  return {
    pricePrecision: countFractionDigits(info?.priceFilter?.tickSize),
    quantityPrecision: countFractionDigits(info?.lotSizeFilter?.basePrecision || info?.lotSizeFilter?.qtyStep)
  };
}

async function fetchOkxPairPrecision(pair) {
  const instId = `${pair.baseSymbol}-${pair.quoteSymbol}`;
  const payload = await fetchJson(`https://www.okx.com/api/v5/public/instruments?instType=SPOT&instId=${encodeURIComponent(instId)}`);
  const info = Array.isArray(payload.data) ? payload.data[0] : null;
  if (!info) return null;
  return {
    pricePrecision: countFractionDigits(info.tickSz),
    quantityPrecision: countFractionDigits(info.lotSz)
  };
}

async function fetchKucoinPairPrecision(pair) {
  const symbol = `${pair.baseSymbol}-${pair.quoteSymbol}`;
  const payload = await fetchJson(`https://api.kucoin.com/api/v2/symbols/${encodeURIComponent(symbol)}`);
  const info = payload.data || null;
  if (!info) return null;
  return {
    pricePrecision: countFractionDigits(info.priceIncrement),
    quantityPrecision: countFractionDigits(info.baseIncrement)
  };
}

async function fetchCoinbasePairPrecision(pair) {
  const productId = `${pair.baseSymbol}-${pair.quoteSymbol}`;
  const payload = await fetchJson(`https://api.exchange.coinbase.com/products/${encodeURIComponent(productId)}`);
  return {
    pricePrecision: countFractionDigits(payload.quote_increment),
    quantityPrecision: countFractionDigits(payload.base_increment)
  };
}

const EXCHANGE_PRECISION_FETCHERS = {
  binance: fetchBinancePairPrecision,
  gate: fetchGatePairPrecision,
  gateio: fetchGatePairPrecision,
  'gate-io': fetchGatePairPrecision,
  indodax: fetchIndodaxPairPrecision,
  mexc: fetchMexcPairPrecision,
  bybit: fetchBybitPairPrecision,
  okx: fetchOkxPairPrecision,
  kucoin: fetchKucoinPairPrecision,
  coinbase: fetchCoinbasePairPrecision,
  'coinbase-exchange': fetchCoinbasePairPrecision
};

async function fetchExchangePairPrecision(pair) {
  const exchangeSlug = normalizeExchangeSlug(pair.exchangeSlug || pair.exchangeName || '');
  const cacheKey = `${exchangeSlug}:${pair.baseSymbol}:${pair.quoteSymbol}`;
  if (exchangePrecisionCache.has(cacheKey)) return exchangePrecisionCache.get(cacheKey);
  const fetcher = EXCHANGE_PRECISION_FETCHERS[exchangeSlug];
  if (!fetcher) {
    return null;
  }
  try {
    ...
    const normalized = {
      priceStep: result.priceStep || formatPrecisionStep(result.pricePrecision),
      quantityStep: result.quantityStep || formatPrecisionStep(result.quantityPrecision)
    };
    if (!normalized.priceStep || normalized.priceStep === '—' || !normalized.quantityStep || normalized.quantityStep === '—') {
      return null;
    }
    exchangePrecisionCache.set(cacheKey, normalized);
    return normalized;
  } catch {
    return null;
  }
...
function buildPairRowFromMarket(pair, precision, options = {}) {
  const currentPrice = Number(options.currentPrice || pair.price || 0);
  const minOrderAmount = Number(options.minOrderAmount || 0);
  const maxOrderAmount = Number(options.maxOrderAmount || 0);
  const priceStep = precision?.priceStep || '—';
  const quantityStep = precision?.quantityStep || '—';
  return {
    pair: `${pair.baseSymbol}/${pair.quoteSymbol}`,
    quoteSymbol: pair.quoteSymbol,
    minTradeQuantity: currentPrice > 0 ? formatTradeQuantity(minOrderAmount / currentPrice) : '—',
    maxTradeQuantity: currentPrice > 0 ? formatTradeQuantity(maxOrderAmount / currentPrice) : '—',
    minOrderAmount: formatPlainAmount(minOrderAmount),
    maxOrderAmount: formatPlainAmount(maxOrderAmount),
    pricePrecision: priceStep,
    quantityPrecision: quantityStep,
    currentPrice: currentPrice > 0 ? formatPriceByStep(currentPrice, priceStep) : '—',
    precisionSourceExchange: pair.exchangeName || '',
    precisionSourceUrl: pair.marketUrl || '',
    precisionSourceResolved: Boolean(precision),
    rowCopyText: ''
  };
}

async function buildUsdtPairRow(detail, marketPairs) {
  const candidates = pickUsdtPairCandidates(detail, marketPairs);
  if (!candidates.length) {
    return {
      ...
      minTradeQuantity: '—',
      maxTradeQuantity: '—',
      minOrderAmount: formatPlainAmount(PAIR_MIN_NOTIONAL_USD),
      maxOrderAmount: formatPlainAmount(PAIR_MAX_NOTIONAL_USD),
      pricePrecision: '—',
      quantityPrecision: '—',
      currentPrice: '—',
      precisionSourceExchange: '',
      precisionSourceUrl: '',
      precisionSourceResolved: false,
      rowCopyText: ''
    };
  }

  for (const pair of candidates) {
    const precision = await fetchExchangePairPrecision(pair);
    if (!precision) continue;
    return buildPairRowFromMarket(pair, precision, {
      minOrderAmount: PAIR_MIN_NOTIONAL_USD,
      maxOrderAmount: PAIR_MAX_NOTIONAL_USD
    });
  }

  return buildPairRowFromMarket(candidates[0], null, {
    minOrderAmount: PAIR_MIN_NOTIONAL_USD,
    maxOrderAmount: PAIR_MAX_NOTIONAL_USD
  });
}

async function buildIdrPairRow(detail) {
  const indodaxPair = pickIndodaxIdrPair(detail, await fetchIndodaxPairs());
  if (!indodaxPair) return null;
  const ticker = await fetchIndodaxTicker(indodaxPair.id);
  const precision = await fetchExchangePairPrecision({
    exchangeSlug: 'indodax',
    exchangeName: 'Indodax',
    exchangePairId: indodaxPair.id,
    baseSymbol: detail.symbol || '',
    quoteSymbol: 'IDR'
  });
  const currentPrice = Number(ticker && ticker.last || 0);
  return buildPairRowFromMarket({
    baseSymbol: detail.symbol || '',
    quoteSymbol: 'IDR',
    price: currentPrice,
    exchangeName: 'Indodax',
    marketUrl: `https://indodax.com/market/${indodaxPair.id}`
  }, precision, {
    currentPrice,
    minOrderAmount: PAIR_MIN_NOTIONAL_IDR,
    maxOrderAmount: PAIR_MAX_NOTIONAL_IDR
  });
}

function buildPairConfigRemark(rows) {
  const valid = (rows || []).filter((row) => row && row.precisionSourceExchange);
  if (!valid.length) return '';
  return valid.map((row) => {
    const label = row.precisionSourceUrl
      ? `<a href="${escapeXml(row.precisionSourceUrl)}" target="_blank" rel="noreferrer">${escapeXml(row.precisionSourceExchange)}</a>`
      : escapeXml(row.precisionSourceExchange);
    if (row.quoteSymbol === 'USDT') return `该币种USDT交易对价格精度和数量精度取自${label}交易所`;
    return `${escapeXml(row.quoteSymbol)}交易对取自${label}交易所`;
  }).join('，') + '。';
}

function buildPairRowCopyText(row) {
  return [
    row.pair,
...
async function buildPairConfigRow(detail) {
  const marketPairs = await fetchCmcMarketPairs(detail.slug, { limit: 80 });
  const rows = [];
  const usdtRow = await buildUsdtPairRow(detail, marketPairs);
  if (usdtRow) rows.push(usdtRow);
  const idrRow = await buildIdrPairRow(detail);
  if (idrRow) rows.push(idrRow);
  rows.forEach((row) => {
    row.rowCopyText = buildPairRowCopyText(row);
  });
  return {
    rows,
    remarkHtml: buildPairConfigRemark(rows)
  };
}
  ...
  const detailed = await Promise.all(matches.map(async ({ item }) => {
    try {
      ...
      return {
        ...
        chainCount: (chainConfig.rows || []).length
      };
    } catch {
      return {
        slug: item.slug,
...
async function runDigest(rootDir, options = {}) {
  ...
  const runRecord = {
    ranAt: nowIso(),
    mode: options.push ? 'push' : 'generate',
  ...
  <header class="topbar">
    <button id="homeBtn" class="topbar-brand" type="button" title="返回主页">
      ...
      <span class="brand-block">
        <span class="brand-eyebrow">Deal Intelligence</span>
      ...
      <div class="col-scroll">
        <div class="sidebar-section">
          ...
          <div class="stats-grid">
            ...
            <div class="stat-card"><div id="statLive" class="stat-value">0</div><div class="stat-label">已补数项目</div></div>
...
function buildTokenConfigHtml(rootDir, options = {}) {
  ...
  const bootstrap = {
    tabs: [
      ...
      { key: 'chain', label: '币链配置表', ready: true },
      { key: 'pair', label: '币对配置表', ready: true },
  ...
  <main class="tool-layout">
    ...
    <section class="token-config-card">
      ...
      <div id="tokenConfigEmpty" class="token-config-placeholder">
        <div class="token-config-placeholder-title">配置表生成已接入</div>
        <p>当前已支持币种配置表、币链配置表和币对配置表。选择币种后会自动抓取并生成对应数据。</p>
      </div>
    </section>
  </main>
