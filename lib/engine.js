...
function mergeExternalDataIntoProject(project, profile, cache) {
  ...
  return {
    ...
    contact: project.contact || (profile ? inferContact(project, profile) : null),
    screening,
...
async function collectLeads(rootDir) {
  ...
  const finalRadarProjects = sortProjectsForAction(
    filterProjectsWithIdentity(radarProjects.filter((project) => !promotedKeys.has(normalizeKey(project.name))))
  );
  const finalFundraisingProjects = sortProjectsForAction(
    filterProjectsWithIdentity(enrichedFundraisingProjects.filter((project) => !promotedKeys.has(normalizeKey(project.name))))
  );
  const finalDexProjects = sortProjectsForAction(
    filterProjectsWithIdentity(enrichedDexProjects.filter((project) => !promotedKeys.has(normalizeKey(project.name))))
  );
  const finalEcosystemProjects = sortProjectsForAction(
    filterProjectsWithIdentity(enrichedEcosystemProjects.filter((project) => !promotedKeys.has(normalizeKey(project.name))))
  );
  ...
  const payload = {
    ...
    fundraisingProjects: finalFundraisingProjects,
    dexProjects: finalDexProjects,
    ecosystemProjects: finalEcosystemProjects
  };
