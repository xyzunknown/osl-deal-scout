...
function parseBinancePrecision(symbolInfo) {
  ...
  return {
    priceStep: String(priceFilter.tickSize || ''),
    quantityStep: String(lotSizeFilter.stepSize || '')
  };
}

async function fetchBinanceSymbolInfo(symbol) {
...
async function buildUsdtPairRow(detail) {
  ...
  const binancePair = {
    ...
    marketUrl: `https://www.binance.com/en/trade/${symbol}_USDT?ref=40896146`
  };
  if (!symbol) {
    return {
  ...
  try {
    const [symbolInfo, tickerPrice] = await Promise.all([
      fetchBinanceSymbolInfo(binanceSymbol),
      fetchBinanceTickerPrice(binanceSymbol)
    ]);
    const precision = symbolInfo ? parseBinancePrecision(symbolInfo) : null;
    if (precision && tickerPrice > 0) {
    ...
    // Binance is the only allowed source for USDT pair metadata.
  }
  return {
    pair: `${symbol}/USDT`,
    quoteSymbol: 'USDT',
...
async function buildPairConfigRow(detail) {
  const rows = [];
  const usdtRow = await buildUsdtPairRow(detail);
...
function buildTokenConfigHtml(rootDir, options = {}) {
  const basePath = String(options.basePath || '').replace(/\/+$/, '');
  const tokenConfigScriptVersion = fs.statSync(path.join(rootDir, 'public', 'token-config.js')).mtimeMs.toFixed(0);
  const tokenConfigStylesVersion = fs.statSync(path.join(rootDir, 'public', 'styles.css')).mtimeMs.toFixed(0);
  const asset = (pathname) => {
    const fullPath = `${basePath}${pathname}`;
    if (pathname === '/token-config.js') return `${fullPath}?v=${tokenConfigScriptVersion}`;
    if (pathname === '/styles.css') return `${fullPath}?v=${tokenConfigStylesVersion}`;
    return fullPath;
  };
